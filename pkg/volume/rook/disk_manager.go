...
type diskManager interface {
	MakeGlobalPDName(disk rook) string
	// Attaches the disk to the kubelet's host machine.
	AttachDisk(disk rookMounter) error
	// Detaches the disk from the kubelet's host machine.
	DetachDisk(disk rookUnmounter, mntPath string) error
	// Creates a rbd image
	CreateImage(provisioner *rookVolumeProvisioner) (r *v1.RookVolumeSource, volumeSizeGB int, err error)
	// Deletes a rbd image
	DeleteImage(deleter *rookVolumeDeleter) error
}

// utility to mount a disk based filesystem
func diskSetUp(manager diskManager, b rookMounter, volPath string, mounter mount.Interface, fsGroup *int64) error {
	globalPDPath := manager.MakeGlobalPDName(*b.rook)
...
func diskTearDown(manager diskManager, c rookUnmounter, volPath string, mounter mount.Interface) error {
	notMnt, err := mounter.IsLikelyNotMountPoint(volPath)
	if err != nil {
		glog.Errorf("cannot validate mountpoint %s", volPath)
		return err
